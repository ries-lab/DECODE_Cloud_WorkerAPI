...
from api.core.filesystem import get_user_modelsystem
...
def create_model(db: Session, user_id: str, model: schemas.ModelCreate):
    try:
        db_model = models.Model(name=model.name, decode_version=model.decode_version, user_id=user_id)
        db.add(db_model)
        db.flush()
...
def delete_model(db: Session, user_id: str, model_id: int):
    ...
    if not db_model or db_model.user_id != user_id:
        return None
    filesystem = get_user_modelsystem(user_id)
